...
def compute_trust(A,b,xdotj,xdotj_nominal,h,min_dist = 1.0,h_min = 1.0): # h > 0
    ...
    # if h < h_min:
    #     print(f"small h: {h}")
    rho_dist = np.tanh(0.05*rho_dist) #np.tanh(3.0*rho_dist); # score between 0 and 1  
    ...
    rho_theta = np.tanh(theta_ns/theta_as*0.8) #0.55 # if it is close to it's nominal, then trust high. if far away from it's nominal, then trust low     
