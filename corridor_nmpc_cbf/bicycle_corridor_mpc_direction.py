...
humans.X[0,0] = -1.7; humans.X[1,0] = -1.3;
...
humans.goals[0,0] =  4.0; humans.goals[1,0] = -1.3;
    ...
    while t < tf:
        ...
        if nominal_sim:
            ...
            try:
                ...
                # print(f"FIXED alpha: { np.min(robot_nominal.alpha_nominal) }, {np.max( robot_nominal.alpha_nominal )}, ACTUAL ALPHA:{mpc_sol.value(alpha1_human)}")
        ...
        if adaptive_sim:
            ...
            try:
                ...
                # print(f"ADAPTIVE alpha: { np.min(robot.alpha_nominal) }, {np.max( robot.alpha_nominal )}, ACTUAL ALPHA:{mpc_sol.value(alpha1_human)}")
