...
dt = 0.05
d_human = 0.5#0.5
U_ref = np.array([0.5, -0.1]).reshape(-1,1)

# robot = holonomic_car(ax, pos = np.array([2.5,-2.5,0]), dt = dt)
robot = unicycle(ax, pos = np.array([1.5,-1.1,np.pi]), dt = dt)
obstacles = []
...
num_people = 20#10
humans = crowd(ax, crowd_center = np.array([0,0]), num_people = num_people, dt = dt_human, horizon = horizon_human, paths_file = 'paths_n20.npy')#social-navigation/
