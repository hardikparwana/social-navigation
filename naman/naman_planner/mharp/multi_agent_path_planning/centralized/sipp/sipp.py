...
class SippPlanner(SippGraph):
    ...
    def compute_plan(self):
        ...
        print("Agent id: {}  Start: {} goal: {}".format(self.name, self.start, self.goal))
        self.open = []
        goal_reached = False
        cost = 1
        
        if self.name == "agent19":
            print("here..")
        ...
        while (not goal_reached):
            ...
            if self.open == []: 
                # Plan not found
                return 0
            if len(self.open) == 0 and self.name == "agent19": 
                print("here2...")
            s = self.open.pop(0)[1]
    ...
    def get_plan(self, failure = False):
        
        if failure: 
            temp_path = {"x": self.goal[0], "y": self.goal[1], "t": 0}
            path_list = [temp_path]
