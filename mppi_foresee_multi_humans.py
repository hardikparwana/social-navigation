...
class MPPI_FORESEE():
    ...
    @staticmethod
    @jit
    def multi_human_dynamics(human_x, other_human_x, robot_x):
    ...
    @staticmethod
    @jit
    def multi_human_dynamics_actual(human_x, other_human_x, robot_x):
    ...
    @staticmethod
    @jit
    def multi_human_sigma_point_expand(sigma_points, weights, other_human_mus, robot_state):
        ...
        def body(i, inputs):
            ...
            mu, cov = MPPI_FORESEE.multi_human_dynamics( sigma_points[:,[i]], other_human_mus, robot_state )
    ...
    def single_sample_rollout(goal, robot_states_init, perturbed_control, human_sigma_points_init, human_sigma_weights_init, human_mus_init, human_covs_init):
        ...
        # indices = jnp.arange(0,MPPI_FORESEE.num_humans)

        @jit
        def loop_over_humans(j, inputs_humans):
            i, cost_sample, robot_state, human_sigma_state, human_sigma_weight, human_sigma_points, human_sigma_weights, human_mus, human_covs = inputs_humans
            ...
            return i, cost_sample, robot_state, human_sigma_state, human_sigma_weight, human_sigma_points, human_sigma_weights, human_mus, human_covs
        ...
        cost_sample = cost_sample + MPPI_FORESEE.cost_goal_coeff * ((robot_state-goal).T @ (robot_state-goal))[0,0] # goal cost

        @jit
        def update_final_cost(j, inputs_humans):
            cost_sample, robot_state, human_sigma_state, human_sigma_weight = inputs_humans
            cost_sample = cost_sample + MPPI_FORESEE.single_sample_state_cost( robot_state, human_sigma_state[:,:,j], human_sigma_weight[:,[j]].T,  goal) / MPPI_FORESEE.num_humans
            return cost_sample, robot_state, human_sigma_state, human_sigma_weight
        cost_sample, _, _, _ = lax.fori_loop(0, MPPI_FORESEE.num_humans, update_final_cost, (cost_sample, robot_state, human_sigma_state, human_sigma_weight) )
    ...
    @staticmethod
    @jit
    def rollout_states_foresee(subkey, robot_init_state, perturbed_control, previous_control, goal, human_init_state_mu, human_init_state_cov):
        ...
        if MPPI_FORESEE.use_gpu:
            @jit
            def body_sample(robot_states_init, perturbed_control_sample, human_sigma_points_init, human_sigma_weights_init, human_mus_init, human_covs_init):
                cost_sample, robot_states_sample, human_sigma_points_sample, human_sigma_weights_sample, human_mus_sample, human_covs_sample = MPPI_FORESEE.single_sample_rollout(goal, robot_states_init, perturbed_control_sample.T, human_sigma_points_init, human_sigma_weights_init, human_mus_init, human_covs_init )
            ...
            cost_total, robot_states, human_sigma_points, human_sigma_weights, human_mus, human_covs = batched_body_sample( robot_states[:,:,0], perturbed_control, human_sigma_points[:,:,:,0], human_sigma_weights[:,:,:,0], human_mus[:,:,:,0], human_covs[:,:,:,0]  )
        else:
            @jit
    ...
    def compute_rollout_costs( self, init_state, goal, human_states_mu, human_states_cov ):
        ...
        t0 = time.time()
        sampled_robot_states, costs, human_sigma_points, human_sigma_weights, human_mus, human_covs = MPPI_FORESEE.rollout_states_foresee(subkey, init_state, perturbed_control, self.U, goal, human_states_mu, human_states_cov)
