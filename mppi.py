...
class MPPI():
    ...
    @staticmethod
    @jit
    def body_sample_chance_constraint(cost_total, states_t, humanX_mu, humanX_cov, goal):            
        def body(j, samples):
            cost_total = samples
            state = states_t[ :,[j] ]   #states_t[2*j:2*j+2,0]
            ...
            # cost_total = cost_total.at[j].set( cost_total[j] + ( 1.0 * (state-goal).T @ (state-goal) + 3/jnp.max( jnp.array([ jnp.min( jnp.square(jnp.linalg.norm(state - humanX_mu, axis=0)) - 1.96**2 * humanX_cov[:,0]  )    ,  0.01]) ) )[0,0] ) # assume human: horizon x 2 x num_humans
            return cost_total
        return lax.fori_loop( 0, MPPI.samples, body, (cost_total) )
    ...
    @staticmethod
    @jit
    def rollout_states_chance_constraint(init_state, perturbed_control, goal, human_states_mu, human_states_cov, human_controls ):
        states = jnp.zeros( (2*MPPI.samples, MPPI.horizon) )
        states = states.at[:,0].set( jnp.tile( init_state, (MPPI.samples,1) )[:,0] )
        humanX_mu = jnp.copy(human_states_mu)
        humanX_cov = jnp.copy(human_states_cov)
        cost_total = jnp.zeros(MPPI.samples)

        # Loop over horizon
        @jit
        def body_horizon(i, inputs):
            states, humanX_mu, humanX_cov, cost_total = inputs     

            # Get cost
            cost_total = MPPI.body_sample_chance_constraint( cost_total, states[:,[i]].reshape((2,MPPI.samples), order='F'), humanX_mu, humanX_cov, goal  )

            # Update state
            states = states.at[:,i+1].set( MPPI.dynamics_step( states[:,[i]], perturbed_control[:, i, :].reshape(-1,1) )[:,0] )
            humanX_mu = humanX_mu + human_controls * MPPI.dt
            humanX_cov = humanX_cov + 0.1 * MPPI.dt * MPPI.dt

            return states, humanX_mu, humanX_cov, cost_total   

        states, humanX_mu, humanX_cov, cost_total = lax.fori_loop( 0, MPPI.horizon, body_horizon, (states, humanX_mu, humanX_cov, cost_total) )
        return states, cost_total
    ...
    def compute_rollout_costs_chance_constraint( self, init_state, goal, human_states_mu, human_states_cov, human_controls ):
        ...
        sampled_states, costs = MPPI.rollout_states_chance_constraint(init_state, perturbed_control, goal, human_states_mu, human_states_cov, human_controls)
