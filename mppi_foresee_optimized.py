...
class MPPI_FORESEE():
    ...
    def rollout_control(init_state, actions):
        ...
        for i in range(MPPI_FORESEE.horizon):
            states = jnp.append( states, MPPI_FORESEE.robot_dynamics_step(states[:,[-1]], actions[i,:].reshape(-1,1)), axis=1 )
        return states
    ...
    @staticmethod
    @jit
    def human_sigma_point_expand_actual(sigma_points, weights, robot_state):
        ...
        def body(i, inputs):
            new_points, new_weights = inputs
            mu, cov = MPPI_FORESEE.human_dynamics_actual( sigma_points[:,[i]], robot_state )
            root_term = get_ut_cov_root_diagonal(cov)           
            temp_points, temp_weights = generate_sigma_points_gaussian( mu, root_term, jnp.zeros((MPPI_FORESEE.human_n, 1)), 1.0 )
            new_points = new_points.at[:,i].set( temp_points.reshape(-1,1, order='F')[:,0] )
            new_weights = new_weights.at[:,i].set( temp_weights.reshape(-1,1, order='F')[:,0] * weights[:,i] )   
    ...
    def rollout_states_foresee(subkey, robot_init_state, perturbed_control, previous_control, goal, human_init_state_mu, human_init_state_cov):
        ...
        indices = jnp.linspace( 0, MPPI_FORESEE.samples, MPPI_FORESEE.samples, dtype=int )
