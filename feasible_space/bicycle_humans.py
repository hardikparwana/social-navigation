...
alpha_qp = cp.Variable(num_people)
alpha_qp_nominal = cp.Parameter(num_people, value=np.ones(num_people)*alpha)
objective2 = cp.Minimize( cp.sum_squares( u2 - u2_ref ) + 10 * cp.sum_squares( alpha_qp - alpha_qp_nominal ) )
A2 = cp.Parameter((n,2))
b2 = cp.Parameter((n,1))
const2 = [A2 @ u2 >= b2]
...
robot = bicycle( ax1[0], pos = np.array([ 1.0, 1.0, np.pi, 2.0 ]), dt = dt, plot_polytope=False )
