...
def compute_smooth_volume_from_states(robot_state, obstacle_states, humans_states, human_states_dot, control_A, control_b):
    A, b = construct_barrier_from_states(robot_state, obstacle_states, humans_states, human_states_dot )
    A2 = jnp.append( -A, control_A, axis=0 )
    b2 = jnp.append( -b, control_b, axis=0 )
    vol = mc_polytope_volume( A2, b2, bounds=control_bound )
    return vol
...
polytope_smooth_volume_from_states_grad = grad( compute_smooth_volume_from_states, argnums=(0,1,2,3) )
...
# with writer.saving(fig1, 'Videos/DU_fs_humans_obstacles_v3.mp4', 100): 
    while t < tf:
        ...
        if use_ellipse:
            ...
            volume_new = compute_smooth_volume_from_states(jnp.asarray(robot.X), obstacle_states, jnp.asarray(humans.X), jnp.asarray(humans.controls), jnp.asarray(control_bound_polytope.A), jnp.asarray(control_bound_polytope.b.reshape(-1,1)) )
            volume_circle2.append(volume_new)
            ax1[2].plot( volume_circle2, 'g--' )
            
            volume_grad_robot, volume_grad_obstacles, volume_grad_humansX, volume_grad_humansU = polytope_smooth_volume_from_states_grad(jnp.asarray(robot.X), obstacle_states, jnp.asarray(humans.X), jnp.asarray(humans.controls), jnp.asarray(control_bound_polytope.A), jnp.asarray(control_bound_polytope.b.reshape(-1,1)) )
            

            ellipse_B2, ellipse_d2, volume_ell = compute_ellipse_from_states(jnp.asarray(robot.X), obstacle_states, jnp.asarray(humans.X), jnp.asarray(humans.controls), jnp.asarray(control_bound_polytope.A), jnp.asarray(control_bound_polytope.b.reshape(-1,1)) )
            volume_grad_robot_ell, volume_grad_obstacles_ell, volume_grad_humansX_ell, volume_grad_humansU_ell = polytope_ellipse_volume_from_states_grad(jnp.asarray(robot.X), obstacle_states, jnp.asarray(humans.X), jnp.asarray(humans.controls), jnp.asarray(control_bound_polytope.A), jnp.asarray(control_bound_polytope.b.reshape(-1,1)))
            # t0 = time.time()
            # print(f"hello vol:{volume_new}, vol ellipse: {volume_ell} grad: {volume_grad_robot.T}, grad_ell: {volume_grad_robot_ell.T}")
            print(f"hello vol:{volume_new}, grad: {volume_grad_robot.T}")
