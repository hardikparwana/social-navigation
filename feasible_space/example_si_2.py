...
with writer.saving(fig, 'Videos/example_si.mp4', 100): 
    ...
    for t in range(num_steps):
        ...
        # CBF constraint: 
        dh_dx1, dh_dx2, h = robot.barrier( obsX1, d_min )
        obstacle_speed = np.zeros((2,1))
        A.value[0,:] = dh_dx1 @ robot.g()
        b.value[0,:] = dh_dx1 @ robot.f() + dh_dx2 @ obstacle_speed + alpha1 * h
        
        dh_dx1, dh_dx2, h = robot.barrier( obsX2, d_min )
        obstacle_speed = np.zeros((2,1))
        A.value[1,:] = dh_dx1 @ robot.g()
        b.value[1,:] = dh_dx1 @ robot.f() + dh_dx2 @ obstacle_speed + alpha1 * h
    ...
    for t in range(num_steps):
        ...
        # CBF constraint: 
        dh_dx1, dh_dx2, h = robot2.barrier( obsX1, d_min )
        obstacle_speed = np.zeros((2,1))
        A.value[0,:] = dh_dx1 @ robot2.g()
        b.value[0,:] = dh_dx1 @ robot2.f() + dh_dx2 @ obstacle_speed + alpha2 * h
        
        dh_dx1, dh_dx2, h = robot2.barrier( obsX2, d_min )
        obstacle_speed = np.zeros((2,1))
        A.value[1,:] = dh_dx1 @ robot2.g()
        b.value[1,:] = dh_dx1 @ robot2.f() + dh_dx2 @ obstacle_speed + alpha2 * h
