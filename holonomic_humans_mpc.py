...
with writer.saving(fig, movie_name, 100): 
    ...
    humans_state = opti_mpc.parameter(2,mpc_horizon*num_people)
    ...
    opti_mpc.subject_to( robot_states[:,0] == robot.X )
    ...
    for k in range(mpc_horizon): # For loop over time horizon
        ...
        for i in range(num_people):
            dist = robot_states[0:2,k] - humans_state[0:2,i]                 
            ...
            # opti.subject_to( cd.mtimes(dist.T, dist) >= 0.3 ) # normal distance constraint  
        ...
        for i in range(len(obstacles)):
            ...
            lambda_bound = max(1.0, 2*h_obstacles[i])                
        ...
        ################ Dynamics ##########################
        opti_mpc.subject_to(  robot_states[0:2,k+1] == robot_states[0:2,k] + robot.f(robot_states[0:2,k])*dt + cd.mtimes(robot.g(robot_states[0:2,k])*dt, robot_inputs[0:2,k]) )
        
        # current state-input contribution to objective ####
        U_error = robot_inputs[0:2,k] - U_ref 
        objective += 10 * cd.mtimes( U_error.T, U_error )
            
        # find control input ###############################             
        objective = 10 * cd.mtimes( U_error.T, U_error ) + 100.0 *(  (alpha_obstacle-alpha_cbf_nominal*np.ones(len(obstacles)))**2 + (alpha_human-alpha_cbf_nominal*np.ones(num_people))**2 )                
    ...
    b_r_barrier = opti_barrier.parameter(4,2)
    A_o_barrier = opti_barrier.parameter(4,2)
    b_o_barrier = opti_barrier.parameter(4,2)
    ...
    while t < tf:
        ...
        for i in range(num_people):
            dist = robot.X[0:2] - human_positions[0:2,i]                 
            h_curr_humans[i] = cd.mtimes(dist.T , dist) - d_min**2
        
        # Find control input
        opti_mpc.set_value(humans_state, human_future_positions)
        opti_mpc.set_value(h_human, h_curr_humans)
        opti_mpc.set_value(h_obstacles, h_obstacles)
