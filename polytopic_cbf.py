...
lambda_o = cp.Variable((1,4))
lambda_r = cp.Variable((1,4))
bnext_r_f = cp.Parameter((4,1))
bnext_r_g = cp.Parameter((4,2))
alpha_cbf = cp.Variable()
U = cp.Variable((2,1))
X = cp.Parameter((2,1))
X_next = cp.Variable((2,1))
dynam_f = cp.Parameter((2,1))
dynam_g = cp.Parameter((2,2))
h = cp.Parameter()
U_ref = cp.Parameter((2,1))

const_cbf = []
const_cbf += [ - lambda_o @ b_o - lambda_r @ bnext_r_f - lambda_r @ bnext_r_g @ U >= alpha_cbf * h ]
const_cbf += [ lambda_o @ A_o + lambda_r @ A_r == 0  ]
const_cbf += [ cp.norm( lambda_o @ A_o) <= 1  ]
const_cbf += [ lambda_o >= 0, lambda_r >= 0 ]
# const_cbf += [ X_next == X + dynam_f + dynam_g @ U  ]
...
obs = rectangle( ax, pos = np.array([1,1])  )
...
while t < tf:
    ...
    Anext, bnext_r_f.value, bnext_r_g.value = robot.polytopic_location_next_state()
    problem_cbf.solve()
